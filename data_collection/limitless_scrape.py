...
def scrape_for_dates_and_url():
    ...
    df_latenight = df[(df[df.columns[1]].str.contains("Late Night #")) | (df[df.columns[1]].str.contains("Late Night Series #")) & (~df[df.columns[1]].str.contains("Late Late Night"))]
...
def update_results(current_results, net_new_results):
    ...
    # updated_plot_df = updated_plot_df.drop_duplicates()
    # logging.info(f"The number of rows after dropping duplicates: {updated_plot_df.shape[0]}")
